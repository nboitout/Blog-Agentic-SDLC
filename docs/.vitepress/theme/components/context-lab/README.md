...
`models.mjs` contains the verified date, source links, rates, context limits, long-context thresholds, and promotional review dates. Review these together when updating a model. The lab displays a freshness notice after 30 days and calls out promotions that need another check. These notices do not update the prices automatically.
